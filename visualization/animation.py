...
def create_gif(fig: Any, path: str, name: str):
    ...
    if isinstance(fig, str):
        # fig is path to folder with single plots
        img, *imgs = [
            Image.open(f) for f in sorted(glob.glob(f"{fig}/*.png"),
...
def animate_skeleton(data: pd.DataFrame, lim_frames: Any = None):
    """ Function to animate a skeleton of the kinetics skeleton data set.

        Important:
            * The skeleton is created based on a predefined adjacency
              matrix (data.KINECT_ADJACENCY)
            * This function only works on the original kinetics skeleton data, not
              the data that is prepared to train the model
    ...
    for i in range(0, len(x)):  # pylint: disable=consider-using-enumerate
        for e_x, e_y in zip(x[np.where(A[i] == 1)], y[np.where(A[i] == 1)]):
            lines.append(ax.plot([x[i], e_x], [y[i], e_y], linestyle='-',
                         linewidth=0.5, color='#c4c4c4', markersize=0))
    ...
    def animate(f):
        ...
        try:
            ...
            if lines_empty:
                for i in range(0, len(x)):  # pylint: disable=consider-using-enumerate
                    for e_x, e_y in zip(x[np.where(A[i] == 1)], y[np.where(A[i] == 1)]):
                        lines.append(ax.plot([x[i], e_x], [y[i], e_y], linestyle='-', linewidth=0.5,
                                     color='#c4c4c4', markersize=0))
            else:
                j = 0
                for i in range(0, len(x)):  # pylint: disable=consider-using-enumerate
                    for e_x, e_y in zip(x[np.where(A[i] == 1)], y[np.where(A[i] == 1)]):
