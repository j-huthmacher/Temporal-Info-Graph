...
class Tracker(object):
    ...
    def __init__(self, ex_name: str = None, sacred_cfg: dict = {}, config: dict = {},
        ...
        if (self.sacred_cfg is not None and
            ...
            self.ex.logger = log
            self.ex.observers.append(MongoObserver(client=client, db_name=self.sacred_cfg["db"]))
    ...
    def model(self):
        ...
        if hasattr(self, "solver") or self.solver is None:
            ValueError("Solver object not provided! I.e. the code to train the model isn't there.")
    ...
    def track(self, mode: Any, cfg: dict = None):
        ...
        if callable(mode):
            ...
            self.cfg = cfg
            self.cfg["start_time"] = datetime.now().strftime("%d.%m.%Y %H:%M:%S")
    ...
    def track_loss(self):
        ...
        if isinstance(self.solver.model, TemporalInfoGraph):
            ...
            E = torch.diag(pos_exp(loss_fn.pos_samples, average=False).mean(dim=1))
            ...
            args = {
                "matrices": [
                    loss_fn.discr_matr.reshape(num_graphs, num_graphs, num_nodes)\
                                      .mean(dim=2).squeeze().detach().numpy(),
                    pos_exp(loss_fn.pos_samples, average=False).detach().numpy(),
                    neg_exp(loss_fn.neg_samples, average=False).reshape(num_graphs, -1, num_nodes)\
            ...
            path = self.local_path + f"JS-MI/discriminator.batch{self.solver.batch}.files"
            if "intermediate_gif" in self.cfg and self.cfg["intermediate_gif"]:
                create_gif(fig, path=self.local_path + f"JS-MI/discriminator.batch{self.solver.batch}.gif")
            elif self.solver.epoch == self.solver.train_cfg["n_epochs"] - 1:
                # Last epoch create gif
                create_gif(path, path=self.local_path + f"JS-MI/discriminator.batch{self.solver.batch}.gif")
                shutil.rmtree(path)
            else:
                Path(path).mkdir(parents=True, exist_ok=True)
                fig.savefig(path+f"/{self.solver.epoch}.discriminator.batch{self.solver.batch}.png")
    ...
    def track_epoch(self):
        ...
        if isinstance(self.solver.model, MLP) and self.track_decision:
            ...
            if "intermediate_gif" in self.cfg and self.cfg["intermediate_gif"]:
                ...
                Path(path).mkdir(parents=True, exist_ok=True)
                fig.savefig(path+f"/{self.solver.epoch}.MLP.decision.boundaries.png")
        ...
        if isinstance(self.solver.model, TemporalInfoGraph) and self.track_decision:
            ...
            if self.solver.epoch == 0:
                ...
                fig.savefig(self.local_path+"TIG.embeddings.pca.final.png")
            plt.close()
            ...
            if "intermediate_gif" in self.cfg and self.cfg["intermediate_gif"]:
                ...
                shutil.rmtree(path)  # Delete the files used for the gif
            else:
                Path(path).mkdir(parents=True, exist_ok=True)
            ...
            if self.solver.epoch == 0:
               ...
               fig.savefig(self.local_path+"TIG.embeddings.tsne.final.png") 
            ...
            if "intermediate_gif" in self.cfg and self.cfg["intermediate_gif"]:
                ...
                Path(path).mkdir(parents=True, exist_ok=True)
                fig.savefig(path+f"/{self.solver.epoch}.TIG.embeddings.tsne.png")
